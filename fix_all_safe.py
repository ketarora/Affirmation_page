...
def main():
    ...
    vibes_old = "title: Row(mainAxisSize: MainAxisSize.min, children: [const NishAffsLogo(size: 24), const SizedBox(width: 8),"
    vibes_new = "title: Row(mainAxisSize: MainAxisSize.min, children: [const NishAffsLogo(size: 28, showText: true), const SizedBox(width: 8),"
    text = text.replace(vibes_old, vibes_new)
...
class Book {
  ...
  Book(name: 'Nothing to Lose But Your Head', author: 'Osho', emoji: '\U0001F98B', tag: 'Zen', grad: [Color(0xFFE0F7FA), Color(0xFFB2EBF2)], file: 'Nothing to Lose but your head.pdf'),
    ...
    # 5. Safe KindleReader replace
    # Find start of KindleReader and StudioView
    start_kr = text.find('class KindleReader extends StatefulWidget {')
    end_kr = text.find('class StudioView extends StatefulWidget {')
    
    if start_kr != -1 and end_kr != -1:
        # Step back to remove the comment block above StudioView if needed, or just replace exact text
        # Let's slice out from start_kr up to the end of the line previous to StudioView
        # actually end_kr is the exact index of 'class Studio'
        
        # We need to find the `// STUDIO VIEW` comment to safely chop
        end_kr_comment = text.rfind('// ════════════════════════════════════════════════════════════════════', start_kr, end_kr)
        
        if end_kr_comment != -1:
            old_kr_chunk = text[start_kr:end_kr_comment]
            new_kr_chunk = '''class KindleReader extends StatelessWidget {
  final Book book;
...
'''
            text = text.replace(old_kr_chunk, new_kr_chunk)
        else:
            print("Kr end comment not found!")
