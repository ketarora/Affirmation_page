...
//  Login • Kindle • Stories • Working Themes • Music Player
//  Mood Tracker • 55×5 Challenge • Vision Board • Streak
//  Every Single Button Working • Real Backend • NishAffs Logo
// ════════════════════════════════════════════════════════════════════
// FIREBASE SWAP (5 lines):
//   1. FirebaseAuth.instance.signInWithEmailAndPassword()
//   2. FirebaseFirestore.instance.collection('posts').snapshots()
//   3. FirebaseFirestore.instance.collection('posts').add(post)
//   4. FirebaseFirestore.instance.doc('users/$uid').set(user)
//   5. Replace ValueListenableBuilder with StreamBuilder
// ════════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════════
//  ASSETS — maps to actual photo files
// ════════════════════════════════════════════════════════════════════
class A {
  static const imgs = [
...
// ════════════════════════════════════════════════════════════════════
//  SOUND PLAYER SERVICE — real progress tracking
// ════════════════════════════════════════════════════════════════════
class SoundPlayerService {
  ...
  void play(int i) {
    ...
    _t?.cancel();
    if (idx.value != i) { elapsed.value = Duration.zero; pos.value = 0; }
    idx.value = i;
  ...
  void stop() {
    _t?.cancel();
    idx.value = -1; pos.value = 0;
    elapsed.value = Duration.zero; isPlaying.value = false;
  }
...
class AppState {
  ...
  List<Map<String, dynamic>> _seedPosts() => [
    ...
    {'id': 'p3', 'user': 'Meera✨', 'avatar': 'M', 'time': '6h ago', 'imgIdx': 14,
     'text': 'I attract opportunities effortlessly. Said this 55 times today and the universe DELIVERED 🎉 Got my dream job callback!',
     'likes': 891, 'comments': ['YAAS QUEEN 👑', 'So happy for you!!', 'This is inspiring 💗', 'How?! Tell me everything!'], 'vibe': 'Abundance'},
...
// ════════════════════════════════════════════════════════════════════
//  NISHAFFS LOGO WIDGET — Custom flower logo
// ════════════════════════════════════════════════════════════════════
...
class _LogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Petals
    final petalPaint = Paint()..color = Colors.white.withOpacity(0.5)..style = PaintingStyle.fill;
    for (var i = 0; i < 6; i++) {
      final angle = i * pi / 3;
      final px = cx + cos(angle) * r * 0.48;
      final py = cy + sin(angle) * r * 0.48;
      canvas.drawOval(
        Rect.fromCenter(center: Offset(px, py), width: r * 0.4, height: r * 0.55),
        petalPaint,
      );
    }
...
// ════════════════════════════════════════════════════════════════════
//  MINI PLAYER STRIP — shows above bottom nav when playing
// ════════════════════════════════════════════════════════════════════
const _soundNames  = ['432Hz Deep Healing','Morning Abundance','Inner Peace Rain','Deep Sleep Delta','Study Focus Beta','Manifest While You Sleep','Chakra Balancing','Self Love Morning'];
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                ...
                ValueListenableBuilder<double>(
                  ...
                  builder: (_, pos, __) => ClipRRect(borderRadius: BorderRadius.circular(2),
                    child: LinearProgressIndicator(value: pos.clamp(0.0, 1.0), minHeight: 3,
...
class _ShellRouteState extends State<ShellRoute> with SingleTickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) => Stack(children: [
    SparkleOverlay(child: AnimatedBuilder(
      animation: _da,
        ...
        bottomNavigationBar: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(20, 0, 20, 14), child: GlassCard(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              _NavBtn(0, _tab, Icons.home_filled, 'Home',    () => _switchTab(0)),
              _NavBtn(1, _tab, Icons.auto_stories_rounded, 'Library', () => _switchTab(1)),
              _NavBtn(2, _tab, Icons.add_circle_rounded, 'Studio',   () => _switchTab(2)),
              _NavBtn(3, _tab, Icons.favorite_rounded, 'Vibes',    () => _switchTab(3)),
              _NavBtn(4, _tab, Icons.person_rounded, 'Me',       () => _switchTab(4)),
            ])))),
        ]),
      ),
    )),
...
class _LeftDrawer extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: const BoxDecoration(
      gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0xFFFFF0F8), Color(0xFFF5E8FF)]),
    ...
    child: SafeArea(child: ValueListenableBuilder(
      ...
      builder: (_, user, __) => ListView(padding: const EdgeInsets.all(20), children: [
        ...
        _dItem(Icons.emoji_events_rounded, '55×5 Challenge', C.pinkDark, () { onClose(); Navigator.push(context, _pageRoute(const Challenge55x5Screen())); }),
        _dItem(Icons.grid_view_rounded, 'Vision Board', C.purple, () { onClose(); Navigator.push(context, _pageRoute(const VisionBoardScreen())); }),
        const SizedBox(height: 20),
        Text('Saved Affirmations', style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.bold, color: C.textDark)),
        const SizedBox(height: 10),
...
// ════════════════════════════════════════════════════════════════════
//  HOME VIEW — every button clickable, different image per affirmation
// ════════════════════════════════════════════════════════════════════
...
class _HomeViewState extends State<HomeView> {
  ...
  @override
  Widget build(BuildContext context) => Stack(children: [
    // Different background image per affirmation index
    Positioned.fill(child: _img((_affIdx * 3 + 10) % 22, w: double.infinity, h: double.infinity)),
    ...
    SafeArea(bottom: false, child: CustomScrollView(physics: const BouncingScrollPhysics(), slivers: [
      SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(22), child: Column(children: [
        ...
        ValueListenableBuilder<int>(
          ...
          builder: (_, mood, __) => mood == -1 ? const _MoodCheckIn() : _MoodBadge(mood)),

        // TODAY'S CARD — different image for each affirmation
        const SizedBox(height: 18),
        AnimatedSwitcher(
          ...
          child: _TodayCard(
            ...
            text: _dailyAffs[_affIdx],
            imgIdx: (_affIdx * 3 + 5) % 22,  // unique image per affirmation!
            liked: _todayLiked,
            onLike: () => setState(() => _todayLiked = !_todayLiked),
          ),
        ),

        // QUICK ACTION PILLS — all clickable
        const SizedBox(height: 20),
        Row(children: [
        ...
        ValueListenableBuilder<int>(
          ...
          builder: (_, streak, __) => streak > 0 ? Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('$streak Day Streak!', style: GoogleFonts.poppins(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.white)),
                Text('Keep manifesting every day ✨', style: GoogleFonts.poppins(fontSize: 11, color: Colors.white.withOpacity(0.85))),
              ])),
              GestureDetector(
        ...
        // CURATED — ALL clickable
        const SizedBox(height: 26),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('Curated for you', style: GoogleFonts.poppins(fontSize: 19, fontWeight: FontWeight.bold, color: C.textDark)),
          GestureDetector(
            onTap: () => Navigator.push(context, _pageRoute(const CuratedListScreen())),
            child: Text('See All →', style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600, color: C.pinkDark))),
        ...
        SizedBox(height: 210, child: ListView.builder(
          ...
          itemCount: 5,
          itemBuilder: (ctx, i) {
            const cats = ['Inner Peace', 'Level Up', 'Healing Era', 'Lucky Girl', 'Deep Sleep'];
            const subs = ['5 Affirmations', '10 Exercises', 'Guided Journal', 'Music + Affs', '8hr Sounds'];
            return GestureDetector(
              ...
              child: Container(width: 150, margin: const EdgeInsets.only(right: 16),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(cats[i], style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.bold, color: C.textDark)),
                  Text(subs[i], style: GoogleFonts.poppins(fontSize: 11, color: C.textSub)),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: [
        Text('How\'s your vibe today? ✨', style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w700, color: C.textDark)),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Today\'s vibe is set!', style: GoogleFonts.poppins(fontSize: 11, color: C.textSub, fontWeight: FontWeight.w600)),
...
class _TodayCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ClipRRect(
    ...
    child: SizedBox(height: 390, width: double.infinity, child: Stack(children: [
      ...
      Positioned(top: 18, left: 18, child: GlassCard(radius: 100, opacity: 0.15,
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('Today\'s Affirmation', style: GoogleFonts.poppins(fontSize: 10, color: Colors.white, fontWeight: FontWeight.w600)),
        ]))),
      Positioned(top: 14, right: 14, child: const NishAffsLogo(size: 26)),
      Positioned(bottom: 22, left: 22, right: 22, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('"$text"', style: GoogleFonts.lora(fontSize: 20, color: Colors.white, fontWeight: FontWeight.w600, height: 1.45)),
        const SizedBox(height: 16),
        Row(children: [
          GestureDetector(
            onTap: onLike,
            child: GlassCard(radius: 100, tint: liked ? C.pinkTheme : Colors.white, opacity: liked ? 0.85 : 0.2,
                    ...
                    child: Stack(children: [
                      ...
                      Center(child: Padding(padding: const EdgeInsets.all(14), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                        ...
                        Text(book.name, textAlign: TextAlign.center,
                          style: GoogleFonts.playfairDisplay(fontSize: 15, color: Colors.white, fontWeight: FontWeight.bold, height: 1.3)),
                      ]))),
...
// ════════════════════════════════════════════════════════════════════
//  KINDLE READER — FIXED natural page flip (swipe left = next page)
// ════════════════════════════════════════════════════════════════════
...
class _KindleReaderState extends State<KindleReader> with SingleTickerProviderStateMixin {
  ...
  /// [fwd] true = go to next page, false = go to previous
  void _flip(bool fwd) {
    if (_flipping) return;
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: GestureDetector(
      // swipe LEFT = next page (natural book reading direction)
      onHorizontalDragEnd: (d) {
        final v = d.primaryVelocity ?? 0;
        if (v < -180) _flip(true);    // swipe left → next
        else if (v > 180) _flip(false); // swipe right → prev
      },
      onTapDown: (d) {
      ...
      child: AnimatedBuilder(
        ...
        builder: (ctx, __) {
          ...
          return Stack(children: [
            back,  // destination page always behind
            // PHASE 1 (v < 0.5): current page flips away from right edge
            if (v < 0.5)
              Transform(
                transform: Matrix4.identity()..setEntry(3, 2, 0.0012)..rotateY(v * pi),
                alignment: Alignment.centerLeft,  // book spine on left
                child: front,
              ),
            // Shadow
            if (v < 0.5)
              Positioned(right: 0, top: 0, bottom: 0,
                child: Container(width: sz.width * (1 - v * 2).clamp(0.0, 1.0),
                  decoration: BoxDecoration(gradient: LinearGradient(
                    begin: Alignment.centerRight, end: Alignment.centerLeft,
                    colors: [Colors.black.withOpacity(0.1 * (1 - v * 2)), Colors.transparent])))),
            // PHASE 2 (v >= 0.5): new page arrives from left
            if (v >= 0.5)
              Transform(
    ...
    child: SafeArea(child: Column(children: [
      ...
      Padding(padding: const EdgeInsets.fromLTRB(24, 14, 24, 0), child: Row(children: [
        GestureDetector(onTap: () => Navigator.pop(context),
          child: const Icon(Icons.arrow_back_ios_new_rounded, size: 16, color: Color(0xFF8B6040))),
        const Spacer(),
        Row(mainAxisSize: MainAxisSize.min, children: [
          const NishAffsLogo(size: 16),
...
// ════════════════════════════════════════════════════════════════════
//  STUDIO — examples, live preview, working post button
// ════════════════════════════════════════════════════════════════════
...
class _StudioViewState extends State<StudioView> {
  ...
  void _showPreview() {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => _PostPreviewSheet(
        text: text, vibe: _vibe, bgIdx: _bgIdx,
        onPost: (target) async {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(bottom: false, child: ListView(padding: const EdgeInsets.fromLTRB(22, 18, 22, 120), children: [
      Row(children: [const NishAffsLogo(size: 30), const SizedBox(width: 10),
        Text('Studio 🎨', style: GoogleFonts.playfairDisplay(fontSize: 26, fontWeight: FontWeight.bold, color: C.textDark))]),
      const SizedBox(height: 6),
      Text('Create your affirmation post ✨', style: GoogleFonts.poppins(fontSize: 13, color: C.textSub)),
      const SizedBox(height: 22),

      // Example taps
      Text('Tap for inspiration:', style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600, color: C.textSub)),
      const SizedBox(height: 10),
...
class _PostPreviewSheet extends StatelessWidget {
  final String text, vibe; final int bgIdx;
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      ClipRRect(borderRadius: BorderRadius.circular(18), child: SizedBox(height: 130, child: Stack(children: [
        ...
        Center(child: Padding(padding: const EdgeInsets.all(18),
...
// ════════════════════════════════════════════════════════════════════
//  COMMUNITY — working search, stories, all post interactions
// ════════════════════════════════════════════════════════════════════
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(bottom: false, child: Column(children: [
      ...
      Padding(padding: const EdgeInsets.fromLTRB(22, 14, 22, 0), child: Row(children: [
        ...
        const Spacer(),
        GestureDetector(
          onTap: () => setState(() { _searching = !_searching; if (!_searching) _q = ''; }),
      ...
      if (_searching) Padding(padding: const EdgeInsets.fromLTRB(22, 10, 22, 0), child: TextField(
        ...
        decoration: InputDecoration(
          hintText: 'Search affirmations, users, vibes...',
          hintStyle: GoogleFonts.poppins(color: C.textSub, fontSize: 14),
      ...
      // Posts feed — real-time via ValueListenableBuilder
      Expanded(child: ValueListenableBuilder(
        ...
        builder: (_, posts, __) {
          ...
          if (filtered.isEmpty) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const SizedBox(height: 12),
            Text(_q.isEmpty ? 'No posts yet! Create one from Studio 🎨' : 'No posts found for "$_q"',
              style: GoogleFonts.poppins(fontSize: 14, color: C.textSub)),
          ]));
...
// ════════════════════════════════════════════════════════════════════
//  STORY VIEWER — real-time progress bars
// ════════════════════════════════════════════════════════════════════
...
class _StoryViewerState extends State<StoryViewer> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _uIdx = widget.initialIndex;
    _prog = AnimationController(vsync: this, duration: const Duration(seconds: 5));
    _start();
  }

  void _start() {
    _prog.reset(); _prog.forward();
    _tmr?.cancel();
    _tmr = Timer(const Duration(seconds: 5), _advance);
  }
            ...
            Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10), child: Row(
              children: List.generate(pages.length, (i) => Expanded(child: Container(
                ...
                child: i < _pIdx
                  ...
                  : i == _pIdx
                    ? AnimatedBuilder(animation: _prog, builder: (_, __) => FractionallySizedBox(
...
// ════════════════════════════════════════════════════════════════════
//  POST CARD — all 4 buttons working
// ════════════════════════════════════════════════════════════════════
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (cmts.isNotEmpty)
        Padding(padding: const EdgeInsets.fromLTRB(20, 0, 20, 4),
          child: Text('💬 ${cmts.first}',
            style: GoogleFonts.poppins(fontSize: 13, color: C.textDark), maxLines: 1, overflow: TextOverflow.ellipsis)),
      if (cmts.length > 1)
      ...
      Padding(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 8),
        child: Divider(color: C.pink2.withOpacity(0.4), height: 1)),
    ]);
  }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(width: 44, height: 5, decoration: BoxDecoration(color: C.pink2, borderRadius: BorderRadius.circular(3))),
          const SizedBox(height: 14),
          Row(children: [const NishAffsLogo(size: 24), const SizedBox(width: 8),
                ...
                : ListView(shrinkWrap: true, children: cmts.map((c) => Padding(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Container(padding: const EdgeInsets.all(10),
                        decoration: BoxDecoration(color: C.pink1, borderRadius: BorderRadius.circular(12)),
          ...
          Row(children: [
            ...
            GestureDetector(
              ...
              child: Container(width: 46, height: 46, decoration: BoxDecoration(shape: BoxShape.circle,
                gradient: const LinearGradient(colors: [C.pinkTheme, C.purple])),
...
// ════════════════════════════════════════════════════════════════════
//  PROFILE — Real working themes, music player with progress
// ════════════════════════════════════════════════════════════════════
    ...
    body: SafeArea(bottom: false, child: Column(children: [
      ...
      Expanded(child: TabBarView(controller: _tc, children: [
        ...
        ValueListenableBuilder(valueListenable: AppState.instance.affs, builder: (_, affs, __) =>
          affs.isEmpty
            ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No entries yet!\nCreate from Studio 🎨', textAlign: TextAlign.center,
                  style: GoogleFonts.poppins(fontSize: 14, color: C.textSub, height: 1.6))]))
            : ListView.builder(
                padding: const EdgeInsets.fromLTRB(20, 16, 20, 120), itemCount: affs.length,
                itemBuilder: (_, i) => Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(16),
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text('${affs[i]['vibe']} · ${(affs[i]['ts'] as String?)?.split('T').first ?? ''}',
                      style: GoogleFonts.poppins(fontSize: 10, color: C.textSub)),
        ...
        ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 120), children: [
          ...
          const SizedBox(height: 6),
          Text('Tap to apply instantly — changes the whole app! ✨', style: GoogleFonts.poppins(fontSize: 13, color: C.textSub)),
          const SizedBox(height: 20),
          ValueListenableBuilder<int>(
            valueListenable: AppState.instance.themeIdx,
            builder: (_, idx, __) => Column(children: List.generate(_appThemes.length, (i) {
        ...
        ListView(padding: const EdgeInsets.fromLTRB(20, 16, 20, 120), children: [
          ...
          Center(child: Column(children: [
            const NishAffsLogo(size: 52),
            const SizedBox(height: 10),
            Text('NishAffs v5.0', style: GoogleFonts.pacifico(fontSize: 20, color: C.pinkDark)),
            const SizedBox(height: 4),
            Text('Made with 💖 in India', style: GoogleFonts.poppins(fontSize: 12, color: C.textSub)),
...
// ════════════════════════════════════════════════════════════════════
//  SOUND CARD — play/pause, real slider, waveform
// ════════════════════════════════════════════════════════════════════
          ...
          return AnimatedContainer(
            ...
            child: Column(children: [
              Row(children: [
                ...
                ValueListenableBuilder<bool>(
                  valueListenable: svc.isPlaying,
                  builder: (_, isp, __) => Container(width: 42, height: 42,
                    decoration: BoxDecoration(shape: BoxShape.circle,
              ...
              // Progress slider + time — only when playing
              if (playing) ...[
                const SizedBox(height: 10),
                ValueListenableBuilder<double>(
                  valueListenable: svc.pos,
                  builder: (_, pos, __) => Column(children: [
                    SliderTheme(
                    ...
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      ValueListenableBuilder<Duration>(valueListenable: svc.elapsed,
                        builder: (_, el, __) => Text(svc.fmt(el), style: GoogleFonts.poppins(fontSize: 10, color: C.textSub))),
...
class _WaveWidgetState extends State<_WaveWidget> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
    animation: _c,
    builder: (_, __) => CustomPaint(size: const Size(double.infinity, 22), painter: _WavePainter(_c.value, widget.color)));
}
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    appBar: AppBar(backgroundColor: Colors.transparent, elevation: 0, centerTitle: true,
      ...
      title: Row(mainAxisSize: MainAxisSize.min, children: [
        const NishAffsLogo(size: 24), const SizedBox(width: 8),
        Text('55×5 Challenge', style: GoogleFonts.poppins(fontSize: 17, fontWeight: FontWeight.bold, color: C.textDark))])),
    body: ValueListenableBuilder(
      valueListenable: AppState.instance.challenge,
      builder: (_, ch, __) {
        if (ch == null || (ch['text'] as String? ?? '').isEmpty) return _ChallengeSetup(tc: _tc);
        return _ChallengeActive(ch: ch);
      }),
  );
}
...
class _ChallengeSetup extends StatelessWidget {
  ...
  const _ChallengeSetup({required this.tc});
  @override
  Widget build(BuildContext context) => SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(children: [
    Container(padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFFFFE4F0), Color(0xFFF0DCFF)]),
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 10),
        Text('Write your affirmation 55 times per day for 5 consecutive days. This technique overwhelms your subconscious and accelerates manifestation.',
          textAlign: TextAlign.center, style: GoogleFonts.poppins(fontSize: 14, color: C.textSub, height: 1.65)),
      ])),
    const SizedBox(height: 24),
    Container(
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20),
        boxShadow: [BoxShadow(color: C.pink2.withOpacity(0.4), blurRadius: 12, offset: const Offset(0, 4))]),
      child: TextField(controller: tc, maxLines: 3,
        style: GoogleFonts.lora(fontSize: 16, color: C.textDark, height: 1.7),
        decoration: InputDecoration(
          hintText: '"I am abundant and deeply loved..."',
          hintStyle: GoogleFonts.lora(fontSize: 14, color: C.textSub.withOpacity(0.55), fontStyle: FontStyle.italic),
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(20), borderSide: BorderSide.none),
          fillColor: Colors.transparent, filled: true, contentPadding: const EdgeInsets.all(18)))),
    const SizedBox(height: 20),
    GestureDetector(
      onTap: () { if (tc.text.trim().isEmpty) return; AppState.instance.startChallenge(tc.text.trim()); },
      child: Container(width: double.infinity, height: 54,
...
class _ChallengeActive extends StatelessWidget {
  ...
  const _ChallengeActive({required this.ch});
  ...
  Widget build(BuildContext context) {
    ...
    final todayCnt   = days[today] as int? ?? 0;
    ...
    return SingleChildScrollView(padding: const EdgeInsets.all(22), child: Column(children: [
      ...
      Container(padding: const EdgeInsets.all(20),
        ...
        child: Text('"${ch['text']}"', textAlign: TextAlign.center,
          style: GoogleFonts.lora(fontSize: 18, color: C.textDark, fontWeight: FontWeight.w600, height: 1.5))),
      const SizedBox(height: 22),
          ...
          Container(width: 46, height: 46, decoration: BoxDecoration(shape: BoxShape.circle,
            ...
            child: Center(child: Text(done ? '✓' : '${i + 1}',
              style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold,
                color: done ? Colors.white : isToday ? C.pinkDark : C.textSub)))),
          const SizedBox(height: 6),
      ...
      Container(padding: const EdgeInsets.all(24),
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24),
          ...
          boxShadow: [BoxShadow(color: C.pink2.withOpacity(0.3), blurRadius: 12)]),
        child: Column(children: [
          Text('Today\'s Progress', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w700, color: C.textSub)),
          const SizedBox(height: 10),
          Text('$todayCnt / 55', style: GoogleFonts.playfairDisplay(fontSize: 42, fontWeight: FontWeight.bold, color: C.pinkDark)),
          const SizedBox(height: 10),
          ClipRRect(borderRadius: BorderRadius.circular(8), child: LinearProgressIndicator(
            value: (todayCnt / 55).clamp(0.0, 1.0), minHeight: 12,
            backgroundColor: C.pink2, valueColor: const AlwaysStoppedAnimation(C.pinkDark))),
          const SizedBox(height: 20),
          GestureDetector(
            onTap: todayCnt >= 55 ? null : () => AppState.instance.incrementChallenge(),
            child: Container(width: double.infinity, height: 54,
              decoration: BoxDecoration(
                gradient: todayCnt >= 55 ? null : const LinearGradient(colors: [C.pinkTheme, C.purple]),
                color: todayCnt >= 55 ? C.pink2 : null,
                borderRadius: BorderRadius.circular(100),
                boxShadow: todayCnt < 55 ? [BoxShadow(color: C.pinkTheme.withOpacity(0.4), blurRadius: 12, offset: const Offset(0, 4))] : []),
              child: Center(child: Text(
                todayCnt >= 55 ? '✓ Done for today! 🌟' : '+ Write it once  (${55 - todayCnt} more to go)',
                style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w700,
                  color: todayCnt >= 55 ? C.textSub : Colors.white))))),
        ])),

      if (daysDone >= 5) ...[
        const SizedBox(height: 20),
        Container(padding: const EdgeInsets.all(20),
          decoration: BoxDecoration(gradient: const LinearGradient(colors: [C.pinkTheme, C.purple]), borderRadius: BorderRadius.circular(22)),
          child: Column(children: [
            const Text('🎉', style: TextStyle(fontSize: 48)),
            Text('Challenge Complete!', style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
            const SizedBox(height: 6),
            Text('You manifested for 5 days straight. The universe has received your intention! ✨',
              textAlign: TextAlign.center, style: GoogleFonts.poppins(fontSize: 13, color: Colors.white.withOpacity(0.9), height: 1.5)),
          ])),
      ],
    ]));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(mainAxisSize: MainAxisSize.min, children: [
          const NishAffsLogo(size: 24), const SizedBox(width: 8),
          Text('Vision Board 🌟', style: GoogleFonts.poppins(fontSize: 17, fontWeight: FontWeight.bold, color: C.textDark))]),
        actions: [IconButton(
          ...
          onPressed: () => showModalBottomSheet(
            ...
            builder: (ctx) => Container(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                const SizedBox(height: 14),
                SizedBox(height: 40, child: ListView.builder(
                  scrollDirection: Axis.horizontal, itemCount: _prompts.length,
                  itemBuilder: (_, i) => GestureDetector(
                    onTap: () => tc.text = _prompts[i],
                ...
                const SizedBox(height: 12),
                TextField(controller: tc, maxLines: 3,
                  style: GoogleFonts.lora(fontSize: 15, color: C.textDark, height: 1.6),
                  decoration: InputDecoration(
                    hintText: 'My dream life includes...',
                    hintStyle: GoogleFonts.lora(fontSize: 14, color: C.textSub.withOpacity(0.5), fontStyle: FontStyle.italic),
                    filled: true, fillColor: C.pink1,
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide.none),
                    contentPadding: const EdgeInsets.all(16))),
                const SizedBox(height: 16),
                GestureDetector(
                  onTap: () { if (tc.text.trim().isEmpty) return; AppState.instance.addVisionCard(tc.text.trim()); Navigator.pop(ctx); },
        ...
        builder: (_, cards, __) => cards.isEmpty
          ...
          : GridView.builder(
              ...
              itemBuilder: (ctx, i) => GestureDetector(
                onLongPress: () => AppState.instance.removeVisionCard(i),
...
class CuratedDetailScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final affs = _aff[title] ?? _aff['Inner Peace']!;
    return Scaffold(
      body: Stack(children: [
        Positioned.fill(child: _img(imgIdx, w: double.infinity, h: double.infinity)),
        Positioned.fill(child: Container(decoration: BoxDecoration(gradient: LinearGradient(
        ...
        SafeArea(child: Column(children: [
          Padding(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12), child: Row(children: [
            const BackButton(color: Colors.white), const Spacer(), const NishAffsLogo(size: 28)])),
          ...
          Expanded(child: ListView.builder(
            padding: const EdgeInsets.symmetric(horizontal: 20), itemCount: affs.length,
            itemBuilder: (_, i) => Padding(
              padding: const EdgeInsets.only(bottom: 14),
              child: GlassCard(radius: 20, opacity: 0.18, padding: const EdgeInsets.all(18),
                child: Row(children: [
                  Container(width: 34, height: 34, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.2)),
                    child: Center(child: Text('${i + 1}', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, color: Colors.white, fontSize: 14)))),
                  const SizedBox(width: 12),
...
class CuratedListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const items = [
      ('Inner Peace', '5 Affirmations',  3), ('Level Up',    '10 Exercises',   4),
      ('Healing Era', 'Guided Journal',  5), ('Lucky Girl',  'Music + Affs',   6),
      ('Deep Sleep',  '8hr Sounds',      7), ('Morning',     '10 min ritual',  8),
      ('Confidence',  'Power Affs',      9), ('Abundance',   'Money Mindset', 10),
    ];
    return Scaffold(
      backgroundColor: C.bg,
      appBar: AppBar(backgroundColor: Colors.transparent, elevation: 0, centerTitle: true,
        ...
        title: Row(mainAxisSize: MainAxisSize.min, children: [
          const NishAffsLogo(size: 24), const SizedBox(width: 8),
          Text('All Collections', style: GoogleFonts.poppins(fontSize: 17, fontWeight: FontWeight.bold, color: C.textDark))])),
      body: GridView.builder(
        padding: const EdgeInsets.all(18), itemCount: items.length,
